...
## 4) 외부 공유용 데모 계정 (학생·교사·진로)

배포 후 다른 사람이 바로 로그인해 볼 수 있게 하려면, DB가 떠 있는 상태에서 서버 루트에서:

```bash
npm run seed:demo
```

- 생성되는 아이디: `student_demo`, `teacher_demo`, `career_demo`
- 기본 비밀번호: `ClassPage2026!` (`.env`에 `DEMO_SEED_PASSWORD`를 두면 그 값으로 해시만 새로 넣고 싶을 때 사용; **이미 있는 username은 건너뜀**)
- 관리자용 `admin`은 이 스크립트에 포함하지 않습니다. 필요하면 위 `bootstrap-admin` 또는 관리자 화면에서 추가하세요.
